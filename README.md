...
**Dashboard (Bảng điều khiển/Màn hình chính):** Màn hình chính của ứng dụng hiển thị tóm tắt các thông tin quan trọng như lịch uống thuốc hôm nay, bữa ăn, vận động, và lịch hẹn sắp tới.

**Notification (Thông báo):** Thông báo được gửi đến thiết bị của người dùng để nhắc nhở về các hoạt động như uống thuốc, ăn uống, vận động, hoặc lịch hẹn.

**CaregiverPlus (Chế độ chăm sóc nâng cao):** Tập hợp các chức năng mở rộng dành cho Caregiver để quản lý nhiều Patients, xem báo cáo, và theo dõi chi tiết.

**Alert (Cảnh báo):** Thông báo quan trọng được gửi đến Caregiver khi Patient có vấn đề sức khỏe nghiêm trọng (ví dụ: triệu chứng mức độ >= 7).

**Link Code (Mã liên kết):** Mã 6 chữ số do Patient tạo ra để chia sẻ với Caregiver khi muốn liên kết tài khoản.

**Health Tracking (Theo dõi sức khỏe):** Quá trình ghi nhận và theo dõi các hoạt động sức khỏe bao gồm bữa ăn, vận động, và triệu chứng.

**Calories (Calo):** Đơn vị đo năng lượng. Calo nạp vào từ thức ăn, calo tiêu thụ từ vận động.

**Report (Báo cáo):** Tờ tổng hợp thống kê sức khỏe theo khoảng thời gian, bao gồm tỷ lệ tuân thủ uống thuốc, tổng calo nạp vào/tiêu thụ, và các thống kê khác.

**Timeline (Dòng thời gian):** Hiển thị các hoạt động theo thứ tự thời gian, thường dùng để xem lịch uống thuốc trong ngày.

**Status (Trạng thái):** Thuộc thể chỉ định tình trạng hiện tại của một mục, ví dụ: Reminder có thể có trạng thái Chờ (PENDING), Đã uống (TAKEN), Bỏ qua (SKIPPED).

**API (Application Programming Interface):** Giao diện lập trình ứng dụng, cho phép ứng dụng mobile giao tiếp với server.

**JWT (JSON Web Token):** Một phương pháp xác thực người dùng, sau khi đăng nhập thành công, server sẽ cấp một token để client lưu và gửi kèm trong các request tiếp theo.

**MongoDB:** Hệ quản trị cơ sở dữ liệu NoSQL được sử dụng để lưu trữ dữ liệu của hệ thống.

**OpenAI:** Dịch vụ trí tuệ nhân tạo được sử dụng để cung cấp trợ lý AI tư vấn sức khỏe, phân tích đơn thuốc, và ước tính calo.

**Cloudinary:** Dịch vụ lưu trữ đám mây được sử dụng để lưu trữ ảnh (avatar, ảnh đơn thuốc).

**Local Storage (Lưu trữ cục bộ):** Không gian lưu trữ trên thiết bị của người dùng (không dùng trong phiên bản hiện tại, dữ liệu được lưu online trên server).
